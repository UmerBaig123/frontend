...
const DataEntryTable: React.FC<DataEntryTableProps> = ({ 
  ...
  // Load data from API on component mount (only once)
  useEffect(() => {
  ...
  // Since we're now saving to localStorage in updateDataState, we don't need this effect

  const refreshData = useCallback(async () => {
    await loadDataFromAPI(setData, setLoading, onDataChange, toast);
  }, [onDataChange, toast]);

  const updateDataState = useCallback((newData: DataItem[]) => {
    setData(newData);
